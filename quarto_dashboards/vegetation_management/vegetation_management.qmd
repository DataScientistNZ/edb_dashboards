...
library(data.table)
library(ggplot2)
source(file.path(here::here(), "R", "00_inflation_data_helper.R"))
...
Some expectations have been expressed that higher expenses in vegetation management could bring a lower SAIDI due to vegetation. That's not what is empirically observed in the data, which seems to show, if anything, a positive correlation between vegetation management expenditure and interruptions caused by vegetation.
...
## section
<h5>Vegetation management opex vs Service interruptions and emergencies opex</h5>

Some expectations have been expressed that higher expenses in vegetation management could bring a lower SAIDI due to vegetation and ultimately lower service interruptions and emergencies opex.  

That sounds like wishful thinking. No clear patterns have been observed between vegetation management cost and quality, and the emergencies opex covers interruptions for any reason, not only vegetation. Vegetation being only ~12% of the identified cause for SAIFI, it's a bit hard to believe in the validity of insights linked to vegetation management one might find observing emergencies opex.

The below presents results for `r latest_year` or for the full period (`r overall_period`) with a vegetation cost normalised using as denominator (note only one favoured option explored here):

* overhead_length


It presents results for emergencies cost normalised using as denominator:
...
dt_tmp <- data.table(dt)[, `:=`(
  "emergencies_opex / overhead_length" = emergencies_opex / overhead_length,
  "emergencies_opex / line_length" = emergencies_opex / line_length,
  "emergencies_opex / nb_connections" = emergencies_opex / nb_connections,
  "emergencies_opex / icp50_line50" = emergencies_opex / icp50_line50)]

dt_edb_avg_tmp <- data.table(dt_edb_avg)[, `:=`(
  "emergencies_opex / overhead_length" = emergencies_opex / overhead_length,
  "emergencies_opex / line_length" = emergencies_opex / line_length,
  "emergencies_opex / nb_connections" = emergencies_opex / nb_connections,
  "emergencies_opex / icp50_line50" = emergencies_opex / icp50_line50)]
...
my_simple_scatter_gplot(dt_tmp[disc_yr==latest_year], x_var="emergencies_opex / overhead_length", 
                        y_numerator="veg_mgt_opex", y_denominator="overhead_length", 
                        groupby="PAT_peergroup", title="", x_log_scale=F) +
...
my_simple_scatter_gplot(dt_tmp[disc_yr==latest_year], x_var="emergencies_opex / line_length", 
                        y_numerator="veg_mgt_opex", y_denominator="overhead_length", 
                        groupby="PAT_peergroup", title="", x_log_scale=F) +
  labs(subtitle = latest_year)
...
my_simple_scatter_gplot(dt_tmp[disc_yr==latest_year], x_var="emergencies_opex / nb_connections", 
                        y_numerator="veg_mgt_opex", y_denominator="overhead_length", 
                        groupby="PAT_peergroup", title="", x_log_scale=F) +
  labs(subtitle = latest_year)
...
my_simple_scatter_gplot(dt_tmp[disc_yr==latest_year], x_var="emergencies_opex / icp50_line50", 
                        y_numerator="veg_mgt_opex", y_denominator="overhead_length", 
                        groupby="PAT_peergroup", title="", x_log_scale=F) +
  labs(subtitle = latest_year)
...
my_simple_scatter_gplot(dt_edb_avg_tmp, x_var="emergencies_opex / overhead_length", 
                        y_numerator="veg_mgt_opex", y_denominator="overhead_length", 
                        groupby="PAT_peergroup", title="", x_log_scale=F) +
...
my_simple_scatter_gplot(dt_edb_avg_tmp, x_var="emergencies_opex / line_length", 
                        y_numerator="veg_mgt_opex", y_denominator="overhead_length", 
                        groupby="PAT_peergroup", title="", x_log_scale=F) +
  labs(subtitle = overall_period)
...
my_simple_scatter_gplot(dt_edb_avg_tmp, x_var="emergencies_opex / nb_connections", 
                        y_numerator="veg_mgt_opex", y_denominator="overhead_length", 
                        groupby="PAT_peergroup", title="", x_log_scale=F) +
  labs(subtitle = overall_period)
...
my_simple_scatter_gplot(dt_edb_avg_tmp, x_var="emergencies_opex / icp50_line50", 
                        y_numerator="veg_mgt_opex", y_denominator="overhead_length", 
                        groupby="PAT_peergroup", title="", x_log_scale=F) +
  labs(subtitle = overall_period)
...
dt_infl <- merge(get_inflation_real_factor("lci_all", quarter=1, min_year=2013),
...
dt_lags <- merge(dt_lags, dt_infl, by.x="disc_yr", by.y="year")
dt_lags[, veg_mgt_opex_real := veg_mgt_opex * opex_escalator]
...
setorderv(dt_lags, c("edb", "disc_yr"), order=1)
dt_lags[, `:=`(veg_mgt_opex_prev1 = lag(veg_mgt_opex, 1),
               veg_mgt_opex_prev2 = lag(veg_mgt_opex, 2), 
               veg_mgt_opex_prev3 = lag(veg_mgt_opex, 3), 
               veg_mgt_opex_prev4 = lag(veg_mgt_opex, 4)), by="edb"]

dt_lags[, `:=`(veg_mgt_opex_real_lag1 = lag(veg_mgt_opex_real, 1),
               veg_mgt_opex_real_lag2 = lag(veg_mgt_opex_real, 2), 
               veg_mgt_opex_real_lag3 = lag(veg_mgt_opex_real, 3), 
               veg_mgt_opex_real_lag4 = lag(veg_mgt_opex_real, 4)), by="edb"]
...
Results for models  
`ln(veg_mgt_opex/overhead_length) ~ ln(veg_saifi_next<n>) [+ edb]`
...
<br>
Results for models  
`ln(veg_saifi) ~ ln(veg_mgt_opex_prev<n>/overhead_length) [+ edb]`
...
```{r, echo=F}
#| title: veg_mgt_opex_prev1

my_reg_var <- "veg_mgt_opex_prev1"
m <- estimatr::lm_robust(as.formula(paste0("I(veg_saifi) ~ I(log(", my_reg_var,"/overhead_length))")), 
                    data = dt_lags[veg_mgt_opex!=0 & (!is.na(get(my_reg_var))) &
...
```{r, echo=F}
#| title: veg_mgt_opex_prev2

my_reg_var <- "veg_mgt_opex_prev2"
m <- estimatr::lm_robust(as.formula(paste0("I(veg_saifi) ~ I(log(", my_reg_var,"/overhead_length))")), 
                    data = dt_lags[veg_mgt_opex!=0 & (!is.na(get(my_reg_var))) &
                                    get(my_reg_var) != 0 & veg_saifi != 0], se_type='HC0')
display_model(m)
...
```{r, echo=F}
#| title: veg_mgt_opex_prev3

my_reg_var <- "veg_mgt_opex_prev3"
m <- estimatr::lm_robust(as.formula(paste0("I(veg_saifi) ~ I(log(", my_reg_var,"/overhead_length))")), 
                    data = dt_lags[veg_mgt_opex!=0 & (!is.na(get(my_reg_var))) &
                                    get(my_reg_var) != 0 & veg_saifi != 0], se_type='HC0')
display_model(m)
...
```{r, echo=F}
#| title: veg_mgt_opex_prev4

my_reg_var <- "veg_mgt_opex_prev4"
m <- estimatr::lm_robust(as.formula(paste0("I(veg_saifi) ~ I(log(", my_reg_var,"/overhead_length))")), 
                    data = dt_lags[veg_mgt_opex!=0 & (!is.na(get(my_reg_var))) &
                                    get(my_reg_var) != 0 & veg_saifi != 0], se_type='HC0')
display_model(m)
...
<br>
Results for models  
`ln(veg_mgt_opex) ~ ln(veg_saifi_next<n>) + ln(overhead_length) [+ edb]`
...
<br>
Results for models  
`ln(veg_mgt_opex/overhead_length) ~ veg_saifi_next<n> [+ edb]`
...
The below repeats the same results as above but accounts for inflation (60% LCI, 40% PPI).  
Accounting for inflation matters as we aim to find a pattern between the variations of cost and the variations of quality. In this context, variations of cost because of the inflationary environment should be accounted for separately. 
...
<br>
Results for models  
`ln(veg_mgt_opex_real/overhead_length) ~ ln(veg_saifi_next<n>) [+ edb]`
...
my_reg_var <- "veg_saifi"
m <- estimatr::lm_robust(as.formula(paste0("I(log(veg_mgt_opex_real/overhead_length)) ~ I(log(", my_reg_var,"))")), 
                    data = dt_lags[veg_mgt_opex_real!=0 & (!is.na(get(my_reg_var))) &
                                    get(my_reg_var) != 0], se_type='HC0')
display_model(m)
...
my_reg_var <- "veg_saifi"
m <- estimatr::lm_robust(as.formula(paste0("I(log(veg_mgt_opex_real/overhead_length)) ~ I(log(", my_reg_var,")) + edb")), 
                    data = dt_lags[veg_mgt_opex_real!=0 & (!is.na(get(my_reg_var))) &
                                    get(my_reg_var) != 0], se_type='HC0')
display_model(m)
...
<br>
Results for models  
`ln(veg_saifi) ~ ln(veg_mgt_opex_real_lag<n>/overhead_length) [+ edb]`
...
```{r, echo=F}
#| title: veg_mgt_opex_real

my_reg_var <- "veg_mgt_opex_real"
m <- estimatr::lm_robust(as.formula(paste0("I(veg_saifi) ~ I(log(", my_reg_var,"/overhead_length))")), 
                    data = dt_lags[veg_mgt_opex_real!=0 & (!is.na(get(my_reg_var))) &
                                    get(my_reg_var) != 0], se_type='HC0')
display_model(m)
...
```{r, echo=F}
#| title: veg_mgt_opex_real_lag1

my_reg_var <- "veg_mgt_opex_real_lag1"
m <- estimatr::lm_robust(as.formula(paste0("I(veg_saifi) ~ I(log(", my_reg_var,"/overhead_length))")), 
                    data = dt_lags[veg_mgt_opex_real!=0 & (!is.na(get(my_reg_var))) &
                                    get(my_reg_var) != 0 & veg_saifi != 0], se_type='HC0')
display_model(m)
...
```{r, echo=F}
#| title: veg_mgt_opex_real_lag2

my_reg_var <- "veg_mgt_opex_real_lag2"
m <- estimatr::lm_robust(as.formula(paste0("I(veg_saifi) ~ I(log(", my_reg_var,"/overhead_length))")), 
                    data = dt_lags[veg_mgt_opex_real!=0 & (!is.na(get(my_reg_var))) &
                                    get(my_reg_var) != 0 & veg_saifi != 0], se_type='HC0')
display_model(m)
...
```{r, echo=F}
#| title: veg_mgt_opex_real_lag3

my_reg_var <- "veg_mgt_opex_real_lag3"
m <- estimatr::lm_robust(as.formula(paste0("I(veg_saifi) ~ I(log(", my_reg_var,"/overhead_length))")), 
                    data = dt_lags[veg_mgt_opex_real!=0 & (!is.na(get(my_reg_var))) &
                                    get(my_reg_var) != 0 & veg_saifi != 0], se_type='HC0')
display_model(m)
...
```{r, echo=F}
#| title: veg_mgt_opex_real_lag4

my_reg_var <- "veg_mgt_opex_real_lag4"
m <- estimatr::lm_robust(as.formula(paste0("I(veg_saifi) ~ I(log(", my_reg_var,"/overhead_length))")), 
                    data = dt_lags[veg_mgt_opex_real!=0 & (!is.na(get(my_reg_var))) &
                                    get(my_reg_var) != 0 & veg_saifi != 0], se_type='HC0')
display_model(m)
...
Results for models  
`ln(veg_mgt_opex_real) ~ ln(veg_saifi_next<n>) + ln(overhead_length) [+ edb]`
...
my_reg_var <- "veg_saifi"
m <- estimatr::lm_robust(as.formula(paste0("I(log(veg_mgt_opex_real)) ~ I(log(", my_reg_var,")) + I(log(overhead_length))")), 
                    data = dt_lags[veg_mgt_opex_real!=0 & (!is.na(get(my_reg_var))) &
                                    get(my_reg_var) != 0], se_type='HC0')
display_model(m)
...
my_reg_var <- "veg_saifi"
m <- estimatr::lm_robust(as.formula(paste0("I(log(veg_mgt_opex_real)) ~ I(log(", my_reg_var,")) + I(log(overhead_length)) + edb")), 
                    data = dt_lags[veg_mgt_opex_real!=0 & (!is.na(get(my_reg_var))) &
                                    get(my_reg_var) != 0], se_type='HC0')
display_model(m)
...
Results for models  
`ln(veg_mgt_opex_real/overhead_length) ~ veg_saifi_next<n> [+ edb]`
...
my_reg_var <- "veg_saifi"
m <- estimatr::lm_robust(as.formula(paste0("I(log(veg_mgt_opex_real/overhead_length)) ~ I(", my_reg_var,")")), 
                    data = dt_lags[veg_mgt_opex_real != 0 & (!is.na(get(my_reg_var))) &
                                    get(my_reg_var) != 0], se_type='HC0')
display_model(m)
...
my_reg_var <- "veg_saifi"
m <- estimatr::lm_robust(as.formula(paste0("I(log(veg_mgt_opex_real/overhead_length)) ~ I(", my_reg_var,") + edb")), 
                    data = dt_lags[veg_mgt_opex_real!=0 & (!is.na(get(my_reg_var))) &
                                    get(my_reg_var) != 0], se_type='HC0')
display_model(m)
...
This section highlights vegetation related cost and outcomes for:  

* Orion NZ
* Network Tasman
* The whole EDB industry
...
dt_w_infl <- merge(dt, dt_infl, by.x="disc_yr", by.y="year")
dt_w_infl[, veg_mgt_opex_real := veg_mgt_opex * opex_escalator]
dt_w_infl[, emergencies_opex_real := emergencies_opex * opex_escalator]
dt_w_infl[, veg_opex_share := veg_mgt_opex / opex]
dt_w_infl[, "veg_mgt_opex_real / overhead_length" := veg_mgt_opex_real / overhead_length]
...
```{r, echo=F}
#| title: veg_mag_opex_real & veg_saifi

my_edb <- "Orion NZ"
my_dual_axis_times_series_gplot(dt_w_infl[edb == my_edb], "disc_yr", 
                                "veg_mgt_opex_real", "veg_saifi", y2_m_dollars=F) +
  labs(subtitle = my_edb)

```

```{r, echo=F}
#| title: normalised veg_mag_opex_real & veg_saifi

my_dual_axis_times_series_gplot(dt_w_infl[edb == my_edb], "disc_yr", 
                                "veg_mgt_opex_real / overhead_length", "veg_saifi",  
                                y1_m_dollars=F, y2_m_dollars=F) +
  labs(subtitle = my_edb)
...
```{r, echo=F}
#| title: veg_mag_opex_real & veg_saidi

my_dual_axis_times_series_gplot(dt_w_infl[edb == my_edb], "disc_yr", 
                                "veg_mgt_opex_real", "veg_saidi", y2_m_dollars=F) +
  labs(subtitle = my_edb)
...
```{r, echo=F}
#| title: veg_opex_share & veg_saifi [lines]

my_dual_axis_times_series_lines_gplot(dt_w_infl[edb == my_edb], "disc_yr", 
                                "veg_opex_share", "veg_saifi", y1_m_dollars=F, y2_m_dollars=F) +
  labs(subtitle = my_edb)

```

```{r, echo=F}
#| title: veg_opex_share & veg_saifi
...
```{r, echo=F}
#| title: veg_mag_opex_real & emergencies_opex_real [bars]

my_single_axis_bar_plot(dt_w_infl[edb == my_edb], x_var="disc_yr", y1_var="emergencies_opex_real", 
                        y2_var="veg_mgt_opex_real") +
  labs(subtitle = my_edb)
...
```{r, echo=F}
#| title: veg_mag_opex_real & emergencies_opex_real

my_dual_axis_times_series_gplot(dt_w_infl[edb == my_edb], "disc_yr", 
                                "veg_mgt_opex_real", "emergencies_opex_real", y2_m_dollars=T) +
  labs(subtitle = my_edb)
...
dt_w_infl_all <- dt_w_infl[, .(veg_mgt_opex_real = sum(veg_mgt_opex_real),
                               emergencies_opex_real = sum(emergencies_opex_real),
                               veg_mgt_opex = sum(veg_mgt_opex),
                               overhead_length = sum(overhead_length),
                               opex = sum(opex),
                               veg_saifi = weighted.mean(veg_saifi, nb_connections),
                               veg_saidi = weighted.mean(veg_saidi, nb_connections)), 
                           by=c("disc_yr")]
dt_w_infl_all[, veg_opex_share := veg_mgt_opex / opex]
dt_w_infl_all[, "veg_mgt_opex_real / overhead_length" := veg_mgt_opex_real / overhead_length]

my_dual_axis_times_series_gplot(dt_w_infl_all, "disc_yr", 
                                "veg_mgt_opex_real", "veg_saifi", y2_m_dollars=F) +
  labs(subtitle = "All EDBs")
...
my_dual_axis_times_series_lines_gplot(dt_w_infl_all, "disc_yr", 
                                "veg_opex_share", "veg_saifi", y1_m_dollars=F, y2_m_dollars=F) +
  labs(subtitle = "All EDBs")
...
my_single_axis_bar_plot(dt_w_infl_all, x_var="disc_yr", y1_var="emergencies_opex_real", 
                        y2_var="veg_mgt_opex_real") +
  labs(subtitle = "All EDBs")
